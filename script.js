...
function generate(event) {
    ...
    // Added if Statement to create password based on character type selected by user. 
    if (passCharString !== "") {
        for (var i = 1; i <= passwordLength; i++) {
        ranPassString = ranPassString + passCharString.charAt(Math.floor(Math.random() * Math.floor((passCharString.length) - 1)));
        }
        // console log of the random password generated by user
        console.log(ranPassString);
        // Random password returned in the password box on HTML page. 
        document.getElementById("password").value = ranPassString;
        }
        else {
            alert("You must use at least one type of character. Please try again.");
        }
